...
- **Plugins** (`PLUGINS=`, #99): at boot, before the `Client`, the Plugin Index is fetched and
  ...
  complete no-op (`Registered 8 slash commands`, as before). The baked-in Warbandeer connector still
  ships in-tree until #100; a `warbandeer` plugin would be skipped for colliding on `/link`.
...
- **Intents come from the Plugin Index JSON, resolved before `createClient()` — never from
  ...
  stays free of both `src/warbandeer/` and `src/config.ts` on purpose — it's read-only data plumbing
  that has to work before either exists in the boot sequence.
