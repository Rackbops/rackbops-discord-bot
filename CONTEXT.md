...
| File | Responsibility |
|---|---|
| `src/index.ts` | Before the `Client` exists at all: `loadPluginIndex()` fetches the Plugin Index, `selectPlugins()` picks the plugins named in `PLUGINS=`, and `collectIntents()` unions their declared intents onto `CORE_INTENTS` — only then is `createClient()` called, since discord.js freezes intents at construction. No plugin code loads here (that's #99); an unset `PLUGINS` behaves exactly as before (Guilds intent only). Then login, then `activate()` — slash-command registration (guild if `DISCORD_SERVER_ID`, else global), the interaction routes (chat commands → `handleCommand`, `/report` modal submits → `handleReportModal`), the scheduler, and `reportUpdateOutcome()` fired un-awaited (an owed `/update` follow-up must not delay startup). Under `resolveBootMode() === "standby"` (#879, confirmed against the daemon since #46) it logs in and attaches **none** of that until `takeOver()` has retired the original — both instances hold the same token and Discord delivers events to both sessions, so a standby with handlers would double every reply. A standby that never reaches `ClientReady` inside `VERIFY_DEADLINE_MS` writes a `failed` marker and exits 1, so the original gets a reason rather than a timeout |
...
| `src/plugins/index.ts` | `loadPluginIndex(url, dataDir, deps?)` — fetches the Plugin Index with a `timeoutMs`-bounded `AbortSignal` (default 5s), validates its shape, and on success atomically caches it to `data/plugins/index.json` wrapped as `{ writtenAt, index }` (`deps.now()` — deliberately distinct from the manifest's own `generatedAt`, which is the plugins repo CI's clock, not this bot's). Any failure (unreachable, timeout, non-200, bad JSON, bad shape, even a cache-write failure after a good fetch) falls back to the cached copy with a `[plugins] index unreachable (…)` warning, or an empty index with a warning if there's no usable cache — **never throws**, so a manifest outage degrades to "no new plugins," never a boot failure. `readFile`/`writeFile` default to real `Bun.file`/mkdir+temp-write+rename (the same shape as `src/warbandeer/storage.ts`, copied rather than imported — `src/plugins/*` stays dependency-free of `warbandeer/` and of `config.ts`) but are injectable for tests |
