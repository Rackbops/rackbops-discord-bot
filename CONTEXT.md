...
| `src/plugins/host.ts` | Loads/wires/activates the installed bundles, inside `activate()` after `takeOver()`. `createHostApi` builds a plugin's `HostApi` — `env` is ONLY the keys the entry declares (never the whole environment), `log` prefixes `[name] `. `loadPlugins` imports each bundle and runs `createPlugin(host)`, isolating a throwing import/createPlugin (recorded, skipped, never fatal). `pluginCommandMap` maps bare→command, dropping a whole plugin's set on any collision with core or an earlier plugin (defence in depth behind `selectPlugins`). `buildCommandBody(prefix, coreJson, map)` = core JSON then each plugin command built via `commandNamer(prefix)` (dropping a builder that produced the wrong name). `pluginTicks` wraps each plugin tick so it runs only while that plugin's `running` flag is set. `activatePlugins` runs each `activate()` in order, isolating a throw. `readPluginState`/`writePluginState`/`buildPluginStateFile` produce `data/plugins/state.json`, preserving the operator-bookkeeping fields (`notifiedVersion`/`skippedVersion`/`remindAt`/`scheduled`/`pendingReport`) from the previous file. `mutatePluginState` is the ONE race-safe runtime read-modify-write of `state.json` (a shared `createKeyedJsonMutator`), used by #103's update tick — the boot `writePluginState` is a one-time whole-file write ordered before any tick fires. All pure/DI'd |
...
- **Plugin updates** (#103): *the operator's copy changes only when the operator says so.* A 15-min
  ...
  panel). It **never** installs, restarts, schedules, or moves a pin (that is #104): it only records
  ...
  installed vs available + the first release note per plugin.
