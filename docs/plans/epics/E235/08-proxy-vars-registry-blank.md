...
5. **Round 1 (correctness-lens review) found group 4 was incomplete: `HTTPS_PROXY` and `NO_PROXY` reserved the upper-case spelling only.** Decision 1's reproduction only checked `HTTPS_PROXY`/`HTTP_PROXY`/`NO_PROXY` and, separately, the lower-case `http_proxy` — it never checked whether Bun's `fetch` also honours lower-case `https_proxy`/`no_proxy`. It does (reproduced against a real `https://` target: an unreachable `https_proxy` breaks the fetch; a lower-case `no_proxy` restores it). Also checked, to close the question rather than leave another gap: a bare `ALL_PROXY`/`all_proxy` is NOT honoured by Bun's `fetch`, so it correctly stays out. Fixed by adding `[https_proxy]=1` and `[no_proxy]=1` to group 4 (now 7 entries, not 5), extending `CORE_SETTINGS` in the behaviour table, and correcting every doc/comment location decision 2 and step 5/deviation 4 touched (all said "five"/"three" — now "seven" throughout). Re-mutation-tested both new entries (removing each independently fails the behaviour table, same mechanism as the original five) in a fresh detached worktree.
6. **Added one test not in the original plan** (committed before item 5's round-1 fix, so it predates it in the branch history but is recorded here after it): a direct assertion in the `RESERVED_KEYS covers` describe (`ops/bot-ops.test.ts`) that every group-4 key is actually present in the parsed `RESERVED_KEYS` set. Found while mutation-testing the widened pin regex (decision 2): nothing else in that describe block references group 4 by name, so reverting the regex to upper-case-only, or dropping a lower-case entry from the bash array, would otherwise pass every existing "RESERVED_KEYS covers" test silently. This test closed that gap; round 2's claims-vs-code reviewer flagged that this item was missing from this section even though the PR body's own summary listed it — omission fixed here, wording-only, no behaviour change.
