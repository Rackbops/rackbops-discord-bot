...
- The swap won't land mid-`data/state.json` write, and it doesn't depend on timing to avoid it. Just before the replacement is created, the original quiesces its scheduler, so no *new* tick or state write starts. A tick already in flight can still be running when the replacement boots — it reads state at module load, seconds after being started, not after it verifies — but that is safe on its own terms: state writes are a temp file plus an atomic rename, so a concurrent reader always gets one whole version or the other, never a torn one. And the replacement only *reads* until it takes over; by the time it writes, the original has been stopped. There are never two writers.
