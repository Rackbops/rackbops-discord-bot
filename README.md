...
- What the quiesce does **not** do is wait for a tick already in flight — it only stops new ones starting. So a tick that was mid-announcement when the swap began can still be running while the replacement is created and verified (a couple of minutes at the outside), and the replacement reads state at module load rather than after it verifies. In the worst case a tick is stopped mid-write when the original is retired, and an announcement it had already posted gets repeated by the new bot. The window is narrow and nothing is corrupted, but it is a real edge — an explicit drain is tracked as issue #154.
