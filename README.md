...
- The quiesce itself doesn't wait for a tick already in flight — it only stops new ones starting, so a tick mid-announcement when the swap began can still be running while the replacement is created and verified. What actually drains it is the signal handler: when the original is retired, it receives `SIGTERM` (same as a plain `docker stop`) and awaits any in-flight announcement or state write, then closes the gateway connection — the drain and that close together are bounded to 8 seconds total, leaving real margin inside the 10-second window before the daemon would `SIGKILL` it — before it actually exits. A write or send that genuinely takes longer than that still loses (the process is gone either way at that point), but the ordinary case — a tick that's a few hundred milliseconds into a state write — now finishes cleanly instead of racing the stop. This isn't specific to a self-update either: a plain `docker stop`, a `bot-ops.sh restart`, or a `compose restart` all drain the same way now.
