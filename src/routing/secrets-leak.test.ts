...
import { mutateRouting, mutateSecrets, readRouting, routingPath, secretsPath } from "./store";
...
describe("a webhook url is only ever in routing.secrets.json", () => {
  test("accepted, refused for its server, unknown to Discord, malformed, unparseable: the token is in the secrets file and nowhere else", async () => {
    ...
    try {
      ...
      // Every request file is gone: none was left queued, none set aside.
      ...
      // The rejected/ folder was never created: a secret-bearing refusal is deleted, not set aside.
      expect(files.has("plugins/requests/rejected")).toBe(false);
      expect([...files.keys()].some((name) => name.includes("rejected"))).toBe(false);
      ...
      for (const [scenario, token] of Object.entries(TOKENS)) {
        ...
        // The accepted one is in the secrets file; every other token is nowhere at all.
        expect(homes, `token of the "${scenario}" request`).toEqual(scenario === "accepted" ? ["file routing.secrets.json"] : []);
      }
